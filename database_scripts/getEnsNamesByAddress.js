...
/**
 * Retrieves all ENS names associated with a given Ethereum address
 * @param {string} address - Ethereum address to look up
 * @returns {Promise<string[]>} Array of ENS names that resolve to this address
 */
async function getEnsNamesByAddress(address) {
  try {
    ...
    try {
      // Query to find all ENS names that resolve to the given address (case-insensitive)
      const result = await client.query(
        'SELECT ens_name FROM ens_table WHERE LOWER(resolved_address) = LOWER($1)',
        [address]
      );
      
      // Return array of ENS names
    ...
    console.error('Error getting ENS names by address:', error);
    return [];
  }
}

module.exports = { getEnsNamesByAddress };
